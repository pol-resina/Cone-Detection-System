...
// class Accumulator
    // public:
        ...
        bool Accumulator::ready() {
            ...
            if (this->enough_imus() && this->enough_points()) {
        ...
        double Accumulator::latest_time() {
            ...
            return this->BUFFER_I.front().time - Config.accumulator.real_time_delay;
        }
    ...
    // private:
        ...
        bool Accumulator::enough_imus() {
            return this->BUFFER_I.size() > Config.accumulator.real_time_delay*Config.accumulator.imu_rate+20;
        }

        bool Accumulator::enough_points() {
            return this->BUFFER_L.size() > 1;
        }
        ...
        void Accumulator::set_initial_time() {
            ...
            this->initial_time = latest_imu_time - Config.accumulator.real_time_delay;
        }
        ...
        bool Accumulator::missing_data(const Points& time_sorted_points) {
            if (time_sorted_points.size() < Config.accumulator.MAX_POINTS2MATCH) return false;
            
            // Check missing 'time' information
            if (time_sorted_points.front().time == 0 and time_sorted_points.back().time == 0) {
                // Remove Initialization
                Config.accumulator.Initialization.times = {};
                Config.accumulator.Initialization.deltas = {Config.accumulator.full_rotation_time};
        ...
        void Accumulator::throw_warning(const Points& time_sorted_points) {
            ...
            ROS_ERROR("Delta has been fixed to %f (s) leading to a fixed %d (Hz) localization.", Config.accumulator.full_rotation_time, (int) (1./Config.accumulator.full_rotation_time));
        }
