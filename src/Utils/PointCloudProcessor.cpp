...
// class PointCloudProcessor
    ...
    // private:
        Points PointCloudProcessor::msg2points(const PointCloud_msg& msg) {
            ...
            if (Config.lidar.LiDAR_type == LIDAR_TYPE::Hesai) return this->hesaimsg2points(msg);
            if (Config.lidar.LiDAR_type == LIDAR_TYPE::Ouster) return this->oustermsg2points(msg);
            if (Config.lidar.LiDAR_type == LIDAR_TYPE::Custom) return this->custommsg2points(msg);
        ...
        // Velodyne specific
            Points PointCloudProcessor::velodynemsg2points(const PointCloud_msg& msg) {         // util
            ...
            double PointCloudProcessor::get_begin_time(const pcl::PointCloud<velodyne_ros::Point>& pcl) {
                ...
                if (Config.lidar.stamp_beginning) return Conversions::microsec2Sec(pcl.header.stamp) + pcl.points.front().time;
        ...
        // Ouster specific
            ...
            double PointCloudProcessor::get_begin_time(const pcl::PointCloud<ouster_ros::Point>& pcl) {
                ...
                if (Config.lidar.stamp_beginning) return Conversions::microsec2Sec(pcl.header.stamp) + Conversions::nanosec2Sec(pcl.points.front().t);
        ...
        Points PointCloudProcessor::temporal_downsample(const Points& points) {
            ...
            for (Point p : points) {
                ...
                bool keep_point = Config.lidar.downsample_rate <= 1 or ++ds_counter%Config.lidar.downsample_rate == 0; 
                if (keep_point and Config.lidar.min_dist < p.norm()) downsampled.push_back(p);
            }
