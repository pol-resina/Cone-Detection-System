    ...
    // public:
        ...
        // Velodyne specific
            Point::Point(const velodyne_ros::Point& p) {
                ...
                if (Config.point.offset_beginning) this->time = (double) p.time;
                // Time offset with respect to end of rotation, i.e. ~= [-0.1, 0]
                else this->time = Config.point.full_rotation_time + (double) p.time;
            }
        ...
        // Ouster specific
            Point::Point(const ouster_ros::Point& p) {
                ...
                // Time offset with respect to beginning of rotation, i.e. ~= [0, 0.1]
                if (Config.point.offset_beginning) this->time = Conversions::nanosec2Sec(p.t);
                // Time offset with respect to end of rotation, i.e. ~= [-0.1, 0]
                else this->time = Config.point.full_rotation_time + Conversions::nanosec2Sec(p.t);
            }
